...
export function App() {
  ...
  const [view, setView] = useState<"public" | "admin">(() =>
    window.location.pathname === "/admin" ? "admin" : "public",
  );
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const [standingData, matchData, teamData] = await Promise.all([
        api.standings(group),
        api.matches(round),
        api.teams(),
      ]);
      setStandings(standingData.tabela);
  ...
  useEffect(() => {
    const onPopState = () =>
      setView(window.location.pathname === "/admin" ? "admin" : "public");
  ...
  async function saveResult(id: number, home: number, away: number) {
    try {
      await api.updateResult(id, home, away);
      await load();
    } catch (reason) {
      setError(
        reason instanceof Error ? reason.message : "Erro ao salvar resultado",
      );
    }
  }
  async function createMatch(data: {
    ...
    try {
      ...
      setError(
        reason instanceof Error ? reason.message : "Erro ao criar partida",
      );
      throw reason;
    }
  }
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#020611] text-white">
      ...
      {error && (
        <div
          role="alert"
          className="bg-red-900 px-6 py-3 text-center text-sm font-bold text-white"
        >
          {error}
        </div>
      )}
